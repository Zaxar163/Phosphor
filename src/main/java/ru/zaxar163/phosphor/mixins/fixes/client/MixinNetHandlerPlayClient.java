...
public abstract class MixinNetHandlerPlayClient implements INetHandlerPlayClient {
	 ...
	 *         teleportation by removing the "Downloading terrain..." screen. This
	 *         will cause the player to see partially loaded terrain rather than
	 *         waiting for the whole render distance to load, but that's also the
	 *         vanilla behaviour for same-dimension teleportation.
	 */
	@Overwrite
	@Override
	public void handleRespawn(SPacketRespawn packetIn) {
		...
		if (packetIn.getDimensionID() != client.player.dimension) {
			...
			world = new WorldClient((NetHandlerPlayClient) (Object) this,
					new WorldSettings(0L, packetIn.getGameType(), false,
							client.world.getWorldInfo().isHardcoreModeEnabled(), packetIn.getWorldType()),
					packetIn.getDimensionID(), packetIn.getDifficulty(), client.profiler);
	...
	// TODO: make this work:
	// @Redirect(method = "handleRespawn", at = @At(value = "INVOKE", target =
	...
	//
	// @Inject(method = "handleRespawn", at = @At(value = "FIELD", target =
	// "Lnet/minecraft/client/network/NetHandlerPlayClient;doneLoadingTerrain:Z",
	// ordinal = 0))
