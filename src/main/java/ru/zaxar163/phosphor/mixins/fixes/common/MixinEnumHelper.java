...
public class MixinEnumHelper {
	...
	@Overwrite
	/*
	 * Everything below this is found at the site below, and updated to be able to
	 * compile in Eclipse/Java 1.6+ Also modified for use in decompiled code. Found
	 * at: http://niceideas.ch/roller2/badtrash/entry/
	 * java_create_enum_instances_dynamically
	 */
	...
	private static <T extends Enum<?>> T makeEnum(Class<T> enumClass, @Nullable String value, int ordinal,
		...
		try {
			return enumClass.cast(
					newInstanceM.invoke(getConstructorAccessor(enumClass, additionalTypes), new Object[] { params }));
		} catch (Throwable e) {
			throw new RuntimeException(e);
		}
	}
	...
	public static void setup() {
		...
		try {
			setup0();
			Method getReflectionFactory = PrivillegedBridge.getMethod(PrivillegedBridge.firstClass(
					"jdk.internal.reflect.ReflectionFactory", "sun.reflect.ReflectionFactory"), "getReflectionFactory");
			reflectionFactory = PrivillegedBridge.ALL_LOOKUP.unreflect(getReflectionFactory).asFixedArity().invoke();
			newConstructorAccessorM = PrivillegedBridge.ALL_LOOKUP
					.unreflect(
							PrivillegedBridge.getMethod(
									PrivillegedBridge.firstClass("jdk.internal.reflect.ReflectionFactory",
											"sun.reflect.ReflectionFactory"),
									"newConstructorAccessor", Constructor.class))
					.asFixedArity();
			newInstanceM = PrivillegedBridge.ALL_LOOKUP.unreflect(
					PrivillegedBridge.getMethod(PrivillegedBridge.firstClass("jdk.internal.reflect.ConstructorAccessor",
							"sun.reflect.ConstructorAccessor"), "newInstance", Object[].class))
					.asFixedArity();
			newFieldAccessorM = PrivillegedBridge.ALL_LOOKUP
					.unreflect(
							PrivillegedBridge.getMethod(
									PrivillegedBridge.firstClass("jdk.internal.reflect.ReflectionFactory",
											"sun.reflect.ReflectionFactory"),
									"newFieldAccessor", Field.class, boolean.class))
					.asFixedArity();
