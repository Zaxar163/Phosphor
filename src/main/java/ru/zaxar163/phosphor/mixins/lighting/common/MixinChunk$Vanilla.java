...
public abstract class MixinChunk$Vanilla {
	...
	/**
	 * Modifies variable k1 before the conditional which decides to propagate
	 * skylight as to prevent it from ever evaluating as true
	 *
	 * @author Angeline
	 */
	@ModifyVariable(method = SET_BLOCK_STATE_VANILLA, at = @At(value = "LOAD", ordinal = 0), index = 11, name = "k1", slice = @Slice(from = @At(value = "INVOKE", target = "Lnet/minecraft/world/chunk/Chunk;relightBlock(III)V", ordinal = 1), to = @At(value = "INVOKE", target = "Lnet/minecraft/world/chunk/Chunk;propagateSkylightOcclusion(II)V")
	...
	/**
	 * Modifies variable j1 before the conditional which decides to propagate
	 * skylight as to prevent it from ever evaluating as true.
	 *
	 * @author Angeline
	 */
	@ModifyVariable(method = SET_BLOCK_STATE_VANILLA, at = @At(value = "LOAD", ordinal = 1), index = 14, name = "j1", slice = @Slice(from = @At(value = "INVOKE", target = "Lnet/minecraft/world/chunk/Chunk;relightBlock(III)V", ordinal = 1), to = @At(value = "INVOKE", target = "Lnet/minecraft/world/chunk/Chunk;propagateSkylightOcclusion(II)V")
