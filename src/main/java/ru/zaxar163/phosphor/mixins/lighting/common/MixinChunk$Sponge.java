...
public abstract class MixinChunk$Sponge {
	private static final String SET_BLOCK_STATE_SPONGE = "setBlockState" + "(Lnet/minecraft/util/math/BlockPos;"
			+ "Lnet/minecraft/block/state/IBlockState;" + "Lnet/minecraft/block/state/IBlockState;"
			+ "Lorg/spongepowered/api/world/BlockChangeFlag;)" + "Lnet/minecraft/block/state/IBlockState;";
	...
	/**
	 * Redirects the construction of the ExtendedBlockStorage in
	 * setBlockState(BlockPos, IBlockState). We need to initialize the skylight data
	 * for the constructed section as soon as possible.
	 *
	 * @author Angeline
	 */
	@Dynamic
	...
	/**
	 * Modifies variable requiresNewLightCalculations before it is used in the
	 * conditional that decides whether or not generateSkylightMap() should be
	 * called. We want it to always take the else branch.
	 *
	 * @author Angeline
	 */
	@Dynamic
	...
	/**
	 * Modifies variable newBlockLightOpacity to match postNewBlockLightOpacity
	 * before the conditional which decides to propagate skylight as to prevent it
	 * from ever evaluating as true.
	 *
	 * @author Angeline
	 */
	@Dynamic
	@ModifyVariable(method = SET_BLOCK_STATE_SPONGE, at = @At(value = "LOAD", ordinal = 1), index = 13, name = "newBlockLightOpacity", slice = @Slice(from = @At(value = "INVOKE", target = "Lnet/minecraft/world/chunk/Chunk;relightBlock(III)V", ordinal = 1), to = @At(value = "INVOKE", target = "Lnet/minecraft/world/chunk/Chunk;propagateSkylightOcclusion(II)V")
	...
	/**
	 * Modifies variable postNewBlockLightOpacity to match newBlockLightOpacity
	 * before the conditional which decides to propagate skylight as to prevent it
	 * from ever evaluating as true.
	 *
	 * @author Angeline
	 */
	@Dynamic
	@ModifyVariable(method = SET_BLOCK_STATE_SPONGE, at = @At(value = "LOAD", ordinal = 0), index = 24, name = "postNewBlockLightOpacity", slice = @Slice(from = @At(value = "INVOKE", target = "Lnet/minecraft/world/chunk/Chunk;relightBlock(III)V", ordinal = 1), to = @At(value = "INVOKE", target = "Lnet/minecraft/world/chunk/Chunk;propagateSkylightOcclusion(II)V")
