...
public class PhosphorHooks {
	...
	private static void transferEntityToWorld(Entity entity, WorldServer oldWorld, WorldServer newWorld, BlockPos pos) {
		...
		if (entity.isEntityAlive()) {
			entity.setLocationAndAngles(pos.getX() + 0.5f, pos.getY(), pos.getZ() + 0.5f, entity.rotationYaw,
	...
	private static void transferPlayerToDimension(EntityPlayerMP player, int dimension, PlayerList manager,
		...
		// We need to capture the current position in case the player disconnected
		// during the teleport,
		// else they will only change dimension, but remain on previous coordinates.
		// See NetHandlerPlayServer.update() - our tick runs in between the capture call
	...
	public void onEntityTravelToDimension(EntityTravelToDimensionEvent event) {
		if (event.getEntity() instanceof EntityPlayerMP) {
			...
			if (fromPos == null
					|| player.getPosition().getDistance(fromPos.getX(), fromPos.getY(), fromPos.getZ()) > 2) {
				...
				if (returnPortal != null) {
					...
					if (server != null) {
						...
						if (toWorld.getBlockState(toPos).getBlock() == Blocks.PORTAL) {
							...
							player.sendStatusMessage(new TextComponentTranslation("phosphor:portal_destroyed"), true);
	...
	public void onPlayerTick(TickEvent.PlayerTickEvent event) {
		if (event.phase == TickEvent.Phase.END && event.side == Side.SERVER) {
			...
			if (entityData.hasKey(SCHEDULED_TELEPORT)) {
				...
				// Fire Forge event - our event handler will ignore it due to SCHEDULED_TELEPORT
				// tag. If this is cancelled, do not teleport at all.
				...
				if (server != null)
					transferPlayerToDimension((EntityPlayerMP) event.player, toDim, server.getPlayerList(),
							BlockPos.fromLong(data.getLong(TO)));
